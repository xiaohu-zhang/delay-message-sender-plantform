...
/**
 * 跟据某个字符串作为key，计算记录对应zk的转发分组，总的组数可以调整，并以参数形式传递
 * 用在客户端
 */
public class ZkCountSlotUtil {
    private static char[] crc_tb = {0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50a5, 0x60c6, 0x70e7, 0x8108, 0x9129,
    ...
    public static int getCRC16(String str, int slotNodeNums) {
        byte[] test = str.getBytes();
        char ch = caluCRC(test);
        int i = (int) ch;
        int modRes = i & 0x3fff;
        int slotRes = modRes / (int) ((0x4000 / slotNodeNums) + 1);
        return slotRes;
    }

    private static char caluCRC(byte[] pByte) {
